...
const SubCateogary = () => {
  ...
  async function deleteData(id) {
    if (window.confirm('Are you sure you want to delete')) {
      let response = await fetch(`http://localhost:8000/subcateogary/${id}`, {
        method: 'delete',
        headers: {
  ...
  async function getData() {
    let response = await fetch('http://localhost:8000/subcateogary', {
      method: 'get',
      headers: {
        'Content-Type': 'application/json',
      },
        ...
        <div className='row'>
          ...
          <div className='col-lg-9 col-md-6'>
            <h5 className='bg-primary text-light text-center p-2'>
              Sub Category
              <Link to='/admin/subcateogary/create'>
                ...
                <tbody>
                  {data.map(item => (
                    <tr key={item.id}>
                      ...
                      <td>
                        <Link to={`/admin/subcateogary/update/${item.id}`} className='btn btn-primary'>
